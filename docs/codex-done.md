...
通知总开关与 Codex 全局 hook 是两层独立控制：

- 暂停通知会保留 hook 和全局工作规则，但所有 `codex-done` 调用都会在产生副作用前安静退出。
- 停用全局 hook 会移除 CodexDone 的 `notify` 接入和全局工作规则，适合长期卸载或恢复 Codex 原始行为。
...
`--status` 输出 `enabled` 或 `disabled`。`--disable` 写入本机配置后立即阻止事件日志、队列、语音、提示音、桌面通知和手机推送，因此已经打开、仍会调用 `codex-done` 的旧线程也会被拦截。`--enable` 恢复所有通知。控制命令本身不会产生完成记录或通知。

菜单栏显示“通知已开启”或“通知已暂停”，并提供直接暂停/恢复按钮。选择“退出 CodexDone”时：

- “仅退出界面”只关闭菜单栏 App，CLI 通知继续有效。
- “暂停通知并退出”先持久化暂停状态，成功后再退出 App。

Web Preview 的状态页和提醒方式页使用同一个 `alert.enabled` 配置。旧配置中没有该字段时默认开启，兼容已有安装。
